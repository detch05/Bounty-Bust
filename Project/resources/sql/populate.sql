...
('What is the difference between a list and a tuple in Python?', 2), -- ID 2 (Bounty 2)
...
INSERT INTO content (description, user_id) VALUES
('Troubleshooting a deployment issue for a Node.js application on Heroku.', 3), -- ID 6
('What is the best way to manage state in React using Redux or Context API?', 5), -- ID 7
('How to optimize query latency in a MySQL database with millions of records?', 4), -- ID 8
('Detailed differences between `INNER JOIN` and `LEFT JOIN` with practical examples.', 1), -- ID 9
('How to implement a pure JavaScript debounce function?', 2), -- ID 10
('What are the best security practices for designing RESTful APIs?', 6), -- ID 11
('How to handle dependency injection in Java projects (Spring/CDI)?', 7), -- ID 12
('What is the role of Containers (Docker/Kubernetes) in modern development?', 8), -- ID 13
('Basic Iptables firewall configuration for a simple web server.', 9), -- ID 14
('How does the Garbage Collector work in Python?', 10), -- ID 15
('Concurrency issues and how to resolve them in Python multi-threading.', 3), -- ID 16
('Strategies for backup and recovery of PostgreSQL databases.', 5), -- ID 17
...
('Consider partial denormalization for read-heavy query performance.', 3), -- ID 21
('Check the use of CTEs or correlated subqueries for better execution plans.', 4), -- ID 22
('Ensure join columns have the same data type and appropriate indexes are present.', 5), -- ID 23
-- Respostas para Bounty 2: 'What is the difference between a list and a tuple in Python?'
('Lists are mutable and used for sequences that might change. Tuples are immutable and faster.', 6), -- ID 24
('Tuples are often used as dictionary keys because of their immutability.', 7), -- ID 25
('The memory consumption of a tuple is generally smaller than a list for the same number of elements.', 8), -- ID 26
-- Respostas para Bounty 3: 'Best practices for designing relational databases?'
('Use 3rd Normal Form (3NF) as a starting point, but optimize for the specific usage scenario.', 9), -- ID 27
('The choice of storage engine (e.g., InnoDB vs MyISAM) can greatly affect performance.', 10), -- ID 28
('Correctly define Primary and Foreign keys to ensure referential integrity.', 1), -- ID 29
-- Respostas para Bounty 4: 'How to center a div in CSS?'
('Use Flexbox with `display: flex; justify-content: center; align-items: center;` on the parent.', 2), -- ID 30
('Alternatively, use CSS Grid for a robust two-dimensional centering solution.', 3), -- ID 31
('The old school way: `margin: auto;` with fixed width and `position: absolute;` with translation.', 4), -- ID 32
-- Respostas para Bounty 5: 'What is backpropagation in neural networks?'
('Backpropagation is an algorithm used to train neural networks by minimizing the error.', 5), -- ID 33
('It calculates the gradient of the loss function with respect to the weights by the chain rule.', 6), -- ID 34
('This process flows backwards from the output layer to the input layer.', 7); -- ID 35
...
INSERT INTO content (description, user_id) VALUES
('Great explanation of the storage engine impact. Did you consider sharding?', 9), -- ID 36 (Comentário ao content 28 - Answer 3)
('Why are you recommending 3NF over BCNF for this specific case?', 10), -- ID 37 (Comentário ao content 27 - Answer 3)
('The memory efficiency point is often overlooked in Python performance discussions.', 6), -- ID 38 (Comentário ao content 26 - Answer 2)
('Does the absolute position method work on all browsers?', 7), -- ID 39 (Comentário ao content 32 - Answer 4)
('Which libraries are commonly used for implementing backpropagation (e.g., TensorFlow or PyTorch)?', 8), -- ID 40 (Comentário ao content 33 - Answer 5)
('Reply to comment 37: BCNF is great, but 3NF offers a better balance when focusing on data update costs.', 1); -- ID 41 (Resposta ao comentário 37)
